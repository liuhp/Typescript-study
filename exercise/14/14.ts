...
Intro:
    ...
    出于某种未知的原因，我们的大多数开发人员离开了公司。我们现在需要积极招聘。
    我们发现有公司利用媒体发布新技术来吸引更多有潜力的候选人。我们需要利用这个机会
    发明并发布一些npm软件包。遵循JS中函数式编程的新趋势，我们决定开发一个实用功能的库。
    这将使我们处于绝望的边缘，因为我们几乎可以确定没有其他人做过类似的事情。
    我们还提供了一些jsdoc以及函数，但有时可能不准确。
 ...
 * 2个入参:返回数组，这个数据是经过特定的mapper函数处理
 *
 * 1 argument passed: returns a function which accepts
 * an input and returns a new array which is a result
 * of input being mapped using original mapper.
 *1个入参: 返回函数，这个函数的入参是数组，出参是原始map函数处理后的数组
 * 
...
// 解答
type mapFun <T>= (item: T, index: number) => {}

export function map <T>(mapper:mapFun<T>, input: T[]): unknown[] | Function | {}[] {
    if (arguments.length === 0) {
        return map;
    }
    if (arguments.length === 1) {
...
// }
// 解答
type filterFun <T>= (item: T, index: number) => {}

export function filter<T>(filterer: filterFun<T>, input:T[]): T[] | Function | {}[] {
    if (arguments.length === 0) {
        return filter;
    }
    if (arguments.length === 1) {
...
// 解答
type reduceFun<T> = (pre: any, cur: T, index: number) => {}
export function reduce<T>(reducer:reduceFun<T>, initialValue: T, input: T[]): T[] | Function |{} {
    if (arguments.length === 0) {
        return reduce;
    }
    if (arguments.length === 1) {
...
export function prop(obj:object, propName: String) {
    ...
    if (arguments.length === 1) {
        return function subFunction(subPropName: string|number) {
            if (arguments.length === 0) {
                return subFunction;
            }
            return obj[subPropName];
